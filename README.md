...
支持 **美股 + A 股 + 港股** 三大市场，基于 **六维评分(基本面,50分) + 缠论(技术面,30分) + 热点(情绪面,20分) = 100分（5:3:2）** 的三维评分体系。
...
| 日线笔 | 20 | 波段方向与最近确认笔 |
| 30分钟线段 | 25 | 入场/离场确认；必须与日线笔同向 |

**入场硬规则：** 日线笔向上且30分钟线段向上才允许标记为"当前可布局"；30分钟数据不足、线段未形成或与日线笔方向冲突时一律"观望"。

**止损/目标（2026-09-01 改为 ATR 动态，替代固定 -8%/+10%）：** 固定比例对波动率差异大的股票失真（低波动票止损形同虚设、高波动票一触即破）。现由 `swing.py` 的 `swing_sl_tp()` 按 ATR14 计算：
- **止损** = max(现价 − 2×ATR, 日线笔低点×0.985)，百分比 clamp 到 **-5%~-11%**（低波动票自动收窄、高波动票自动放宽）
- **目标** = 现价 + 3×ATR，百分比 clamp 到 **+8%~+25%**，且目标比例 ≥ 止损比例×1.5（保证盈亏比 ≥ 1.5:1）
- ATR 缺失（日K不足15根）时回退固定 -8%/+10% 兜底

30分钟K线统一通过 Yahoo Finance 获取；Yahoo 不可用时显式标记“数据缺失”，不会降级为60分钟或日线数据。A股和港股都可用：
