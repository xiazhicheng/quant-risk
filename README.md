...
> **V2.0（2026-09-08）**：主看 **A 股 + 港股**（美股已移出维护）。默认**纯技术波段推荐**——方向引擎从缠论整体替换为**道氏理论**（摆动点 + 高点/低点序列判趋势），四维评分（日线趋势30 + 日线量价资金25 + 日线道氏20 + 30分钟道氏25 = 100）；每只标的附**同花顺式三tab简介**（📋简况 / 📊财务 / 🎯看点 / 📌近期动态）。旧六维评分+缠论 5:3:2 保留在 `--mode value` legacy 路径。
...
A股与港股默认运行**纯技术波段模式**，目标持有几天至 1-2 周。评分不使用 PE、ROE、负债率、营收或净利，也不会因这些基本面字段否决标的。
...
**入场硬规则：** 日线道氏与30分钟道氏同为上升趋势才标记"当前可布局"；30分钟数据不足、趋势未确认或方向冲突时一律"观望"。

**止损/目标（2026-09-01 改为 ATR 动态，替代固定 -8%/+10%）：** 固定比例对波动率差异大的股票失真（低波动票止损形同虚设、高波动票一触即破）。现由 `swing.py` 的 `swing_sl_tp()` 按 ATR14 计算：
- **止损** = max(现价 − 2×ATR, 摆动低点×0.985)，clamp 到 **-5%~-11%**（低波动票自动收窄、高波动票自动放宽）
- **目标** = 现价 + 3×ATR，百分比 clamp 到 **+8%~+25%**，且目标比例 ≥ 止损比例×1.5（保证盈亏比 ≥ 1.5:1）
- ATR 缺失（日K不足15根）时回退固定 -8%/+10% 兜底
...
```bash
uv run scripts/recommend.py --market hk                # 港股波段（默认）
uv run scripts/recommend.py --market cn                # A股波段（默认）
...
uv run scripts/backtest_swing.py --record report/recommend-cn-20260908.md  # 记录TOP10（回测闭环）
uv run scripts/backtest_swing.py --report             # T+5/T+10/T+20 胜率统计
```
