...
分析以 **六维评分(基本面,50分) + 缠论(技术面,30分) + 热点(情绪面,20分) = 100 分（5:3:2）** 加权。六维评分由 6 个大师视角维度等权求和，缠论作为技术面指标（周线定大势、日线定买卖点），热点作为情绪面指标。满分 **100 分**。
...
4. **池内百分位排名** — 映射到 1~5 → 加权得分 = fb_w(百分位×10) + ch_w(百分位×6) + hot_w(百分位×4)（5:3:2）
