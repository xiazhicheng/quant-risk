...
- **📊 一图胜千言（2026-07-23 新增）**：所有分析报告优先使用 Mermaid 图而非纯表格。目前包含两类 Mermaid 图：
  - **产业链全景图**：上中下游节点 + 基本面数据嵌入 + 竞争格局，让读者一眼看懂标的在产业链中的位置和卡脖子环节
  - **六维评分全景图**：6个维度各自展示大师视角、质疑、答疑，连接线标注 ✅/❌ 结论，总分和行业漏斗结果在中心节点
