...
- **数据分析方法**: 三维评分 → 总得分 100 分。六维评分(基本面,60分) + 缠论(技术面,20分) + 热点(情绪面,20分)。六维评分由 6 个大师视角维度（生意质量/护城河/管理层/最大风险/文明趋势/估值）1-10 分等权求和，满分 60 分。投资理念：**六维评分为基础、缠论辨结构、热点看情绪**。
...
- **📌 投资理念铁律（2026-07-21 用户明确）**：**六维评分为基础、缠论辨结构、热点看情绪**。代码实现和所有分析必须严格贯彻此优先级——六维评分权重最大(60分)且是入场决策的第一筛选条件，缠论作为技术面择时参考(20分)，热点作为情绪面参考(20分)。**任何时候都不能只看技术面买入，基本面不合格的标的必须一票否决。** 评分阈值（≥70强烈关注）已经体现了"六维评分好+缠论好+热点好"三高共振才强烈推荐的逻辑。
...
- **标的池筛选**: 四层流程：①宏观扫描(板块排名→候选池) → ②中观过滤(市值/股价硬约束) → ③基本面一票否决(营收< -30%或净利< -30%或PE严重负值等8条直接淘汰，贯彻"六维评分为基础") → ④微观评分(六维评分×12+缠论×4+热点×4，满分100)
- **评分系统 V3（2026-07-22 重构，六维评分）**:
  ...
  - 三维评分合成：`total = fb_w(六维基本面,60分) + ch_w(缠论技术面,20分) + hot_w(热点情绪面,20分) = 100分`
...
- **输出格式「四大师视角对抗 + 选股→定价→择时」三段式（2026-07-22 重构为四大师框架）**:
  ...
  - 满分 100 分，TOP10 表列名：`📊六维评分(60分) | 🔧缠论(20分) | 🔥热点(20分)`
...
三维评分体系（100 分制，2026-07-22 重构为六维评分）：
...
5. 情绪面：`hot_w = 百分位×4`，技术面：`ch_w = 百分位×4`
