...
- **A股日K**: 腾讯 (前复权) > 新浪 (免鉴权) > 百度 (带MA) > TickFlow (最终兜底)
...
- **AI 偏见自查清单（2026-07-22 新增）**:
  ...
  - 每条标注在报告中的对应处理方式
- **`portfolio_report.py` 一键报告（2026-07-22 新增；2026-09-21 技术面缠论→道氏；2026-09-23 调仓全道氏化）**:
  - **2026-09-23 大改（用户要求"调仓建议逻辑按道氏理论改、不要大模型专家辩论"）**：
    - **调仓裁决改为道氏信号驱动**：新增 `_dow_verdict()`，按道氏三步映射动作——日线下降+收盘破前低→🔴清仓；下降未破前低→🟡反弹至前高减半；日线震荡→🟡持有观察（边界定生死）；日线上升+动能走弱预警→🟡减半（滞后性纪律）；上升+缩量→🟡持有；上升+放量健康→🟢持有。**六维评分不再参与调仓**。
    - **投委会辩论已删除**：`_pm_master_debate()`（163 行）与 `_safe_float()` 一并移除，逐只改为「🎯 道氏持仓裁决」（方向/量能/边界/裁决/触发价）。
    - **调仓路线图不再硬编码**：原买入端写死"01308 海丰国际 / 02698 乐舒适 / 03888 减仓 9600HKD"（换持仓即全错），改为 `_load_top_picks()` 自动读取 `report/recommend-{cn,hk}-<date>-daily.md` 的每市场 TOP2（跳过「观望」），无报告时提示先跑 daily_run。
    - 「组合优化建议」（六维评分<30 且亏损>20% 才止损）→「🎯 道氏持仓裁决总表」；「AI偏见自查清单」（硬编码标的条目）→「🔍 数据源自查」。
    - `dow_detail_output()` 返回值新增结构化字段 `direction/previous_low/previous_high/stage/health/divergence/vol_ratio` 供裁决消费；「基本面分析」标题改为「基本面背景（不参与调仓裁决）」，无产业链数据时不再输出空标题。
  - 整合模块（当前）：组合总览 → 调仓路线图（道氏）→ 逐只（基本信息 → 基本面背景 → 核心财务 → **道氏持仓裁决** → 市场情绪 → 技术面）→ 道氏持仓裁决总表 → 数据源自查
  ...
  - 仍保留的 legacy 路径：`--sixdim-json` / `--sixdim-render`（JSON 管道，`_render_analysis` 输出六维评分表+漏斗+镜子测试），未随本次改动清理
  - 镜子测试序号自动递增（①②③④⑤）
## 分析框架
