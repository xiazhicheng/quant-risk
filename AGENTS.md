...
- **📄 主题文档索引（细节按需读取，不占常驻上下文）**：
  ...
  - `docs/agents/value-legacy.md` — **legacy value 模式**（六维评分/四大师/缠论/镜像测试/行业漏斗/产业链Mermaid）。仅 `--mode value`、tech_chan、推荐脚本 formatter 投委会时读取。**portfolio_report.py 已于 2026-09-23 全道氏化（调仓裁决改为道氏信号驱动、投委会辩论已删除），不再属本文件范围。**
...
- **数据分析方法（当前默认）**：A股与港股采用纯技术波段筛选，**持有周期不限定为 1-2 周——可以是几天、1 个月，若是真正的好标的持有 1 年也可**。总分 100 分：日线趋势 30 + 日线量价/资金 25 + 日线道氏 20 + 30 分钟道氏 25。评分只使用行情、成交量、资金和道氏趋势（摆动点）数据——基本面不参与评分与估值比较；但另设**基本面门禁（只否决不加分）**，详见下方「基本面门禁」条与 `docs/agents/swing-system.md`。
...
- **滞后性纪律=扣分+降档双轨（2026-09-24 grill Q7/Q12，升级原纯降档版）**：三条可量化纪律在 `swing_score_one` 代码化——①动能走弱（价创新高 MACD 柱收缩）日线趋势 **-6**；②高位放量滞涨（60日区间≥85%分位 + 量比>2x + 当日涨幅<1%）量价 **-6**；③高位横盘（高位区连续≥10日振幅<4%）日线趋势 **-4**。扣分写回维度分并在 reason 标注 `⚠️XX扣N分`（评分扣分可见），明细进 `result["discipline"]`；状态封顶**谨慎布局**且规则引擎 ALLOW 也压不回"当前可布局"（BLOCK/EXIT 等更严裁决不受影响）；预警不抢道氏转势确认的活——跌破前低才由道氏给离场。
- **基本面门禁（2026-09-24 grill Q1/Q5/Q6/Q11，核心实现 `scripts/quantrisk/gate.py`）**：基本面**只否决不加分**——不进 100 分评分（评分保持纯技术：趋势30/量价25/日线道氏20/30m道氏25）；评分完成后对候选（≤80只/市场）+ 持仓**后置**拉财务评估，不扫全池。A股严：归母净利为负/负债率>70%/营收同比<-20%/ST·退市；港股松：退市风险/负债率>80%；**金融业（名称含 银行/证券/保险/信托）仅豁免负债率规则**（负债=存款/保费是商业模式，净利/营收/ST 规则照常，`notes` 留痕并在报告透出）；财务数据缺失 **fail-open 不否决**（回测归"未评估"，不硬凑分组）。触发即从 TOP10 剔除并渲染 `⛔ 基本面门禁否决：<规则>`，回测记录 `veto` 字段做通过组/否决组对比（Q13 通过线）；**持仓只标"不加仓"，卖出裁决仍完全归道氏 `_dow_verdict()`**；单股分析（analyze_swing.py）同源评估，veto 时状态封顶⛔禁止新建仓。
...
- **禁止直接粘贴脚本输出**——LLM 必须：
  ...
  2. 结合外部宏观环境，判断脚本给出的道氏持仓裁决（离场/减半/持有观察/持有）是否需要调整，调整意见必须标注为「LLM 补充意见」
...
**调仓路线图 Mermaid 图**（道氏裁决驱动的卖出端：清仓/减半 → T+2到账 → 买入端自动取自当日 recommend 报告 TOP2，已跳过「观望」，不硬编码）
...
按以下结构逐只输出，**不含六维评分表、行业漏斗、镜子测试、投委会辩论（均已取消）**；调仓裁决只由道氏信号决定（`portfolio_report.py` 的 `_dow_verdict()`）。
...
**🎯 道氏持仓裁决**（方向/边界/量能/裁决/触发价 — 由 `_dow_verdict()` 生成，**基本面不参与调仓裁决**）

**操作建议**: 🔴离场 / 🟡减半 / 🟡持有观察 / 🟡持有 / 🟢持有
...
- 各标的操作提醒（为什么这么操作）
- 🌍 宏观环境（联网检索，作为调仓决策的背景补充，每条数据标注来源 URL）
...
├── portfolio.py             持仓诊断工具
├── portfolio_report.py      持仓完整报告（道氏调仓裁决+产业链背景+道氏技术面+情绪面）🔥
...
**腾讯 K 线域名（2026-08-12 修复）**：`http://web.ifzq.gtimg.cn`（HTTP+web 前缀）已失效 501，修复为 `_tencent_kline_get()` 多域名降级：`https://ifzq.gtimg.cn` → `https://proxy.finance.qq.com/ifzqgtimg/`。**HTTPS + 非 web 前缀是硬要求**，改回 `web.ifzq.gtimg.cn` 会让道氏摆动点判定全挂（波段评分只拿最低基准分 = K 线源全挂的信号）。
...
总分 100 分：**日线趋势 30 + 日线量价/资金 25 + 日线道氏 20 + 30 分钟道氏 25**。三档状态：当前可布局 / 谨慎布局 / 观望。详细评分维度、道氏三步、降级链、ATR 止损移动止盈、上车/离场条件见 `docs/agents/swing-system.md`。legacy 六维评分框架（`--mode value`）见 `docs/agents/value-legacy.md`。
