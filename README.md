...
支持 **美股 + A 股 + 港股** 三大市场，基于 **六维评分(基本面,60分) + 缠论(技术面,20分) + 热点(情绪面,20分) = 100分** 的三维评分体系。

## 投资理念

> **六维评分(基本面,60分) + 缠论(技术面,20分) + 热点(情绪面,20分) = 100分**
>
...
│  ├── 基本面否决  fundamental_veto(营收/净利/PE/负债率等8条)│
		│  ├── 六维评分    _raw_score_one(6维度,等权求和,满分60分)   │
		│  ├── 热点评分    hot_score(满分20分)                       │
		│  └── 缠论评分    chan_score(满分20分)     │
│                                              │
│  分析层                                        │
...
分析以 **六维评分(基本面,60分) + 缠论(技术面,20分) + 热点(情绪面,20分) = 100 分** 加权。六维评分由 6 个大师视角维度等权求和，缠论作为技术面指标，热点作为情绪面指标。满分 **100 分**。

| 维度 | 满分 | 类型 | 子维度 | 核心问题 |
|------|:----:|:----:|--------|---------|
| 📊 六维评分 | 60 | 基本面 | 生意质量/护城河/管理层/最大风险/文明趋势/估值 | 估值合理吗？盈利质量如何？财务健康吗？ |
| 🔧 缠论 | 20 | 技术面 | MA排列/MACD/买卖点/周线大势 | 结构位置在哪？有买卖点信号吗？ |
| 🔥 热点 | 20 | 情绪面 | 板块资金排名/个股资金流向/成交量/动量 | 是否在市场主线？ |

**评分流程（4 层过滤）：**
1. **中观过滤** — 市值≥50亿、股价≥1元（硬约束，不通过即淘汰）
2. **基本面一票否决** — 营收<-30%、净利<-30%、PE<-10、负债率>90%、ROE<0 等 8 条规则直接淘汰
3. **原始分计算** — `_raw_score_one`(6维度各1-10分) + `hot_score` + `chan_score`，基础分 2.0
