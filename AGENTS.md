...
2. **本地 JSON 配置文件** — `portfolio.json`（项目根目录）
3. **外部记忆系统** — AgentMemory（通过 MCP JSON-RPC 协议调用 `memory_recall`，**查询方法详见知识库** `/Users/xiazhicheng/project/llm_wiki/articles/agentmemory-mcp-query-method.md`），搜索关键词 `portfolio` / `持仓` / `holding`
4. **询问用户** — 以上均无数据时，要求用户自行提供持仓信息
...
| scripts/recommend_hk.py | 港股选股推荐脚本 |
