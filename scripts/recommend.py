...
输出内容:
    - 波段模式（A股/港股默认）：四维评分100（日线趋势30+量价资金25+日线道氏20+30分钟道氏25）
      + 道氏三步 + 三档状态（可布局/谨慎/观望，带触发价/止损/移动止盈）
      + 基本面门禁（只否决不加分：评分后置评估，触发即从 TOP10 剔除并标⛔）
    ...
    - 价值模式（--mode value）：兼容旧版评分链（四大师/镜子测试/Checklist/行业漏斗），
      仅此路径保留，非默认输出
