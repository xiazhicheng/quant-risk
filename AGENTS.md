...
- **数据分析方法**: 二维评分 → 总得分 100 分。基本面 50 分(50%权重) + 技术面 50 分(50%权重)。技术面由两个子分组成：热点(原始分×5) + 缠论(原始分×5)，各 1-5 分，合计 10~50 分。投资理念：**基本面为主、技术面为辅、结合当前热点**。
...
| 维度 | 权重 | 满分 | 子维度 | 核心问题 |
|------|:----:|:----:|--------|---------|
| 📊 基本面 | 50% | 50 | — | 估值合理吗？盈利质量如何？财务健康吗？ |
| 🔧 技术面 | 50% | 50 | 🔥 热点(×5) + 🔧 缠论(×5) | 是否在市场主线？结构位置在哪？有买卖点信号吗？ |

**满分**：基本面 50 + 技术面 50 = **100 分**（技术面 = 热点子分 + 缠论子分）

**评分流程**：
1. 原始分计算（`fb_score`/`hot_score`/`chan_score`）— 基础分 2.0，各维度加减分
2. 池内百分位排名（`percentile_score_all`）— 映射到 1~5
3. 加权得分：`fb_w = 百分位×10`，`hot_w = 百分位×5`，`ch_w = 百分位×5`
4. 技术面总分 = `hot_w + ch_w` = 10~50 分
